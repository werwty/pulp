...
def handle_worker_heartbeat(event):
    ...
    if event_info['worker_name'].startswith(TASKING_CONSTANTS.RESOURCE_MANAGER_WORKER_NAME):
        resourcemanager_lock, created = TaskLock.objects.get_or_create(name=event_info['worker_name'], lock=TaskLock.RESOURCE_MANAGER)
        resourcemanager_lock.timestamp = timezone.now()
        resourcemanager_lock.save()
